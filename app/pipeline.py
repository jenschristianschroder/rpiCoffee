...
Executes the full coffee pipeline:
  ...
  2. localml   → classify coffee type
  3. locallm   → generate a witty statement
  4. localtts  → synthesize speech (WAV)
  5. remotesave → save results + CSV to remote service (non-fatal)
...
async def run_pipeline() -> dict[str, Any]:
    ...
    # ── Step 1: Classify via localml ─────────────────────────────
    classification = await MLClient.classify(sensor_data)

    if classification is None:
        result["steps_skipped"].append("localml")
        result["steps_skipped"].append("locallm")
        result["steps_skipped"].append("localtts")
        result["steps_skipped"].append("remotesave")
        result["error"] = "Classification unavailable – pipeline stopped"
        return result

    result["label"] = classification["label"]
    result["confidence"] = classification["confidence"]
    result["steps_completed"].append("localml")

    # ── Step 2: Generate text via locallm ────────────────────────
    ...
    result["steps_completed"].append("locallm")

    # ── Step 3: Synthesize speech via localtts ───────────────────
...
async def run_pipeline_streaming() -> AsyncGenerator[str, None]:
    ...
    classification = await MLClient.classify(all_sensor_data)

    if classification is None:
        result["steps_skipped"].extend(["localml", "locallm", "localtts", "remotesave"])
        result["error"] = "Classification unavailable – pipeline stopped"
        yield _sse("result", result)
        return
    ...
    if llm_result is None:
        result["steps_skipped"].extend(["locallm", "localtts", "remotesave"])
        result["error"] = "Text generation unavailable – pipeline stopped after classification"
        yield _sse("result", result)
        return
    ...
    if audio_bytes is None:
        result["steps_skipped"].extend(["localtts", "remotesave"])
        result["error"] = "Speech synthesis unavailable – pipeline stopped after text generation"
        yield _sse("result", result)
        return
